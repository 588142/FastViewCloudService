...
for vol in vols:
    ...
    if not os.path.exists(fp):
        fp = os.path.join(ws, vol, vol.replace('-', '') + '.html')
    ...
    print(f'{vol:25s} | {size:>3}KB | {lines:>4}行 | {zh:>5}字 | 本科{appx}/11 硕士{masters}/8 | 3D={int(has_3d)} Chart={int(has_chart)} | 结尾={ends_ok} | {status}')